...
def avg_size_trace(tr):
    ...
    if not tr.randsol in AVG_SIZE:
        ...
        curr_cum_size, curr_iter = sum([len_trace(tr.get_trace()[1]) for _ in range(1000)]), 1000
        ...
        while abs(1-prev_avg_size/curr_avg_size) > 0.001: # coverged within 1% tolerance?
            ...
            curr_cum_size += sum([len_trace(tr.get_trace()[1]) for _ in range(1000)])
            curr_iter += 1000
            curr_avg_size = 1.0 * curr_cum_size / curr_iter
...
def len_trace(trace): # takes into account size of trace entry of list type
    
    return sum(len(trace[k][1]) if isinstance(trace[k][1], list) else 1 for k in trace.keys())
